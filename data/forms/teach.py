...
#校验输入的设备信息字段不能为空
class TeachModelForm(ModelForm):
    class Meta:
        model = models.Teacher
        fields = "__all__"
        widgets = {
            'numid':forms.TextInput(attrs={'class':'form-control','placeholder':'工号'}),
            'name':forms.TextInput(attrs={'class':'form-control','placeholder':'姓名'}),
            'grade':forms.Select(attrs={'class':'form-control','placeholder':'所带班级'}),
            'birth':forms.TextInput(attrs={'class':'form-control','placeholder':'生日'}),
            'gender':forms.Select(attrs={'class':'form-control','placeholder':'性别'}),
            'study_time':forms.TextInput(attrs={'class':'form-control','placeholder':'入职时间'}),
